...
from deepmask.utils.load_helper import load_pretrain
import deepmask.models as models
...
parser.add_argument('--resume', default='/media/HardDisk_new/wh/code/pysot-master/pysot-master/deepmask/pretrained/DeepMask.pth.tar',
                    type=str, metavar='PATH', help='path to checkpoint')
parser.add_argument('--img', default='/media/HardDisk_new/wh/code/pysot-master/pysot-master/deepmask/data/ori1 th.png',
...
def main():
    ...
    scales = [2**i for i in range_end(args.si, args.sf, args.ss)] # i=[-2.5:0.5:0.5]
    ...
    for i in range(masks.shape[2]):
        ...
        res[:, :, 0] = masks[:, :, i] * 255 + (1 - masks[:, :, i]) * res[:, :, 0]
        # res[:, :, 1] = masks[:, :, i] * 255 + (1 - masks[:, :, i]) * res[:, :, 1]
