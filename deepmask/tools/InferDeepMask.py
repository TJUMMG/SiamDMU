...
class Infer(object):
    def __init__(self, model, nps=500, scales=(1.,),
                 ...
                 iSz=160, device='cpu', timer=False):
    ...
    def forward(self, img):
        ...
        for inp in imgPyramid:
            ...
            # plt.show()
            outMask_ = outMask.sigmoid().cpu().data.numpy()
            self.mask.append(outMask_)
            # torch.cuda.empty_cache()

            outScore_ = outScore.sigmoid().cpu().data.numpy()
            self.score.append(outScore_)
