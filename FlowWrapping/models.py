...
try:
    from FlowWrapping.networks.resample2d_package.resample2d import Resample2d
    from FlowWrapping.networks.channelnorm_package.channelnorm import ChannelNorm

    from FlowWrapping.networks import FlowNetC
    from FlowWrapping.networks import FlowNetS
    from FlowWrapping.networks import FlowNetSD
    from FlowWrapping.networks import FlowNetFusion

    from FlowWrapping.networks.submodules import *
except:
...
class FlowNet2C(FlowNetC.FlowNetC):
    ...
    def _Conv_Pre3(self, x):
        out_conv1 = self.conv1(x)
        out_conv2 = self.conv2(out_conv1)
        out_conv3 = self.conv3(out_conv2)
        return out_conv2, out_conv3

    def _Conv_For4(self, x, out_conv2):
        out_conv3_1 = self.conv3_1(x)
        ...
        flow2_up = self.upsample1(flow2 * self.div_flow)
        return flow2_up

    def warp(self, input, flow):
        warpping_img = self.resample1(input, flow)[:, :, 0:127, 0:127] * self.rgb_max
        return warpping_img
    ...
    def forward(self, inputs):
        with torch.no_grad():
            ...
            out_conv2a, out_conv3a = self._Conv_Pre3(x1)
            ...
            out_conv2b, out_conv3b = self._Conv_Pre3(x2)

            # Merge streams
            out_corr = self.corr(out_conv3a, out_conv3b) # False
            out_corr = self.corr_activation(out_corr)

            # out_corr = self.corr_activation(self.corr(out_conv3a, out_conv3b))
            ...
            flow2_up = self._Conv_For4(in_conv3_1, out_conv2a)
