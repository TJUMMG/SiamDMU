...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
    ...
    for idx, (tracker_name, auc) in  \
        ...
        plt.plot(thresholds, np.mean(value, axis=0),
                color=COLOR[idx], linestyle=LINE_STYLE[idx],label=label, linewidth=float(0.3*idx)+1)
    # ax.legend(loc='upper right', labelspacing=0.2, fontsize=9, frameon=False)
    ax.legend(loc='lower left', labelspacing=0.2, handlelength=3.5, frameon=False)
    ...
    if precision_ret:
        ...
        for idx, (tracker_name, pre) in \
            ...
            plt.plot(thresholds, np.mean(value, axis=0),
                     color=COLOR[idx], linestyle=LINE_STYLE[idx], label=label, linewidth=float(0.3*idx)+1)
        ax.legend(loc='lower right', labelspacing=0.2, handlelength=3.5, frameon=False)
        # ax.legend(loc='lower right', labelspacing=0.2, fontsize=9, bbox_to_anchor=(1, 0.65), frameon=False)
        ax.autoscale(enable=True, axis='both', tight=True)
        xmin, xmax, ymin, ymax = plt.axis()
        ax.autoscale(enable=False)
        ymax += 0.03
